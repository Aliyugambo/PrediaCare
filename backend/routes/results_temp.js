 ...
 * Share a test result with the patient (doctor shares the lab result with patient)
 * Permission: ADD_RESULTS (doctor)
 */
router.post('/:id/share', checkPermission(PERMISSIONS.ADD_RESULTS), async (req, res) => {
  try {
    const doctorId = req.session.doctorId;
    const { id } = req.params;
    const { share_notes } = req.body;
    ...
    const notes = share_notes ? 'Shared with patient: ' + share_notes : 'Shared with patient';
    await connection.execute(`
      UPDATE results SET status = 'reviewed', notes = CONCAT(IFNULL(notes, ''), '\n', ?) WHERE id = ?
    `, [notes, id]);
    ...
    res.json({
      success: true,
      message: 'Test result shared with patient successfully'
    });
  } catch (error) {
    console.error('Error sharing result:', error);
    res.status(500).json({ 
      success: false, 
      message: 'Failed to share result with patient' 
    });
  }
});

/**
 * GET /api/results/doctor/lab-results
 * Get test results from lab staff that need doctor review
 * Permission: ADD_RESULTS (doctor)
 */
router.get('/doctor/lab-results', checkPermission(PERMISSIONS.ADD_RESULTS), async (req, res) => {
  try {
    const doctorId = req.session.doctorId;
    ...
    const [pendingCount] = await connection.execute(`
      SELECT COUNT(*) as count FROM results WHERE doctor_id = ? AND status = 'completed'
    `, [doctorId]);
    
    const [sharedCount] = await connection.execute(`
      SELECT COUNT(*) as count FROM results WHERE doctor_id = ? AND status = 'reviewed'
    `, [doctorId]);
    ...
    res.json({
      ...
      sharedWithPatients: sharedCount[0].count,
      results: results.map(result => ({
        id: result.id,
        patientId: result.patient_id,
        patientName: result.patient_name,
...
/**
 * GET /api/results/doctor/all
 * Get all test results created by or for the doctor's patients
 * Permission: ADD_RESULTS (doctor)
 */
router.get('/doctor/all', checkPermission(PERMISSIONS.ADD_RESULTS), async (req, res) => {
  try {
    const doctorId = req.session.doctorId;
    ...
    let query = 'SELECT r.id, r.patient_id, r.test_name, r.test_type, r.result_data, r.report_file, r.file_name, r.status, r.notes, r.result_date, r.created_at, u.name as patient_name, u.email as patient_email FROM results r JOIN users u ON r.patient_id = u.id WHERE r.doctor_id = ?';
...
/**
 * GET /api/results/medical-reports
 * Get medical reports/examination records for the patient
 * Permission: VIEW_RESULTS (patient)
 */
router.get('/medical-reports', checkPermission(PERMISSIONS.VIEW_RESULTS), async (req, res) => {
  try {
    const patientId = req.session.userId;
    const { limit = 50, offset = 0 } = req.query;
    ...
    const [reports] = await connection.query(`
      ...
      JOIN users u ON d.user_id = u.id
      WHERE e.patient_id = ?
      ORDER BY e.examination_date DESC LIMIT ${Math.floor(Number(limit)) || 50} OFFSET ${Math.floor(Number(offset)) || 0}
    `, [patientId, parseInt(limit), parseInt(offset)]);
    
    const [countResult] = await connection.execute(
      'SELECT COUNT(*) as total FROM examinations WHERE patient_id = ?',
      [patientId]
    );
